...
graph <- ggplot() +
  geom_bar(data = subset(full.long, variable %in% c("freep", "covidp", "noncovidp")), 
           ...
           alpha = 0.75) +
  geom_bar(data = subset(full.long, variable %in% c("dosesp")), 
           aes(x = date, y = val, fill = variable), stat = "identity", width = 1, 
           alpha = 0.50) +
  ...
  geom_line(data = subset(full.long, variable %in% c("secvaccrate_de")),
            aes(x = date, y = val, color = "col3"), size = 1.1) +
  geom_line(data = subset(full.long, variable %in% c("secvaccrate_us")), 
  ...
  geom_text_repel(aes(label = round(val, 1), x = date, y = val), 
                  ...
                  segment.colour = "transparent") +
  geom_text_repel(aes(label = round(val, 1), x = date, y = val), 
                  data = full.long[full.long$date == max(full.long$date) & full.long$variable == "dosesp",], 
                  size = 3.5, fontface = "bold", color = "chartreuse3", nudge_x = 1, nudge_y = 0, 
                  segment.colour = "transparent") +
  scale_fill_manual(values = c("freep" = "gray68", 
                    ...
                    labels = c("freep" = "Free ICU capacity, 7-day roll. avg.",
                               ...
                               "dosesp" = "Avail. of doses for full vacc. (% of pop.)")) +
  scale_color_manual(values = c(col1 = "darkorange2", 
                     ...
                     labels = c("7-day incidence",
                                ...
                                "Vaccination rate (full) Germany", 
                                "Vaccination rate (full) US")) +
